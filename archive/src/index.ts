...
import { asNumber, asRecord, asString, never } from "./safe";

export { ArchiveName, ArchiveProfile, ArchiveEgress, ArchiveClient };
...
async function resolveName(env: Env, name: string, ip: string) {
  const stub = env.ARCHIVE_NAMES.get(env.ARCHIVE_NAMES.idFromName(name.toLowerCase()));
  return stubJson(stub, "https://name/resolve", { method: "POST", body: JSON.stringify({ name, ip }) });
}

async function loadProfile(env: Env, uuid: string, ip: string) {
  const stub = env.ARCHIVE_PROFILES.get(env.ARCHIVE_PROFILES.idFromName(uuid));
  return stubJson(stub, "https://profile/load", { method: "POST", body: JSON.stringify({ uuid, ip }) });
}
...
function lastStatusOf(value: unknown): number {
  ...
  return asNumber(state?.lastStatus) || asNumber(rec?.status) || 0;
}
...
async function userPayload(env: Env, id: string, ip: string, version: Version): Promise<Response> {
  ...
  if (!uuid) {
    ...
    const resolved = asRecord(await resolveName(env, name, ip));
    ...
    if (!uuid) {
      ...
      if (infra && classified !== "missing" && status !== 404 && status !== 204) return infra;
      if (status === 404 || status === 204 || classified === "missing") {
        return err(version, 404, "Not Found", `No user with the name '${name}' was found`);
      }
      if (infra) return infra;
      return err(version, status >= 400 ? status : 502, "Bad Gateway", `Failed to fetch user with the name '${name}' from Mojang`);
    }
  }

  const profile = asRecord(await loadProfile(env, uuid, ip));
  ...
  if (!session || !username) {
    ...
    if (infra && pClass !== "missing" && pStatus !== 404 && pStatus !== 204) return infra;
...
async function uuidPayload(env: Env, id: string, ip: string, version: Version): Promise<Response> {
  const nameMode = version === "v4" ? "loose" : "strict";
  const name = asUsername(id, nameMode);
  if (!name) {
    return version === "v1"
  ...
  async fetch(request: Request, env: Env, _ctx?: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if ((version === "v1" || version === "v2" || version === "v4") && (method === "user" || method === "uuid") && arg) {
        ...
        const res =
          method === "uuid"
            ? await uuidPayload(env, arg, ip, version)
            : await userPayload(env, arg, ip, version);
