...
# @returns {[err, json]} - An error or an array of service statuses.
export health = ->
  get("https://status.mojang.com/check", json: true)
...
export usernameToUuid = (username, secs = -1) ->
  ...
  get("https://api.mojang.com/users/profiles/minecraft/#{username}#{time}", json: true)
...
# ]
#
# @param {[string]} usernames - Maximum of 100 usernames to query.
# @throws {400} - When given an empty or null username.
# @returns {[err, json]} - An error or JSON response with UUIDs.
export usernameToUuidBulk = (usernames...) ->
  post("https://api.mojang.com/profiles/minecraft", usernames, json: true)
...
# Fallback base64-encoded png texture for "steve" skins.
...
# Fallback base64-encoded png texture for "alex" skins.
